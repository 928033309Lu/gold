...
mastLayer.loadMast = function (dataList) {
  mastLayer.remove()
...
mastLayer.zoomToFeature = function (mast_id, t, y) {
  ...
  gwmap.viewer.flyTo(entities, {
    offset: new WindEarth.HeadingPitchRange(0, t || -0.8, y || 15000)
  })
}
...
function addLabelLayer (item) {
  ...
  const labelEntity = new WindEarth.BillBoardFeatureEntity({
    ...
    positions: [item.longitude || item.coordinate_x, item.latitude || item.coordinate_y, item.heights],
    styleOptions: {
      mast: item,
      url: itemLabel.url,
      scale: 1,
...
function getLabel (item) {
  ...
  fillRoundRect(ctx, 0, 0, 150, 70, 10, item.complete ? '#80BFFE' : '#D8F3EF')
  ctx.beginPath()
  ctx.moveTo(70, 70)
  ctx.lineTo(85, 70)
  ctx.lineTo(77, 77)
  ctx.fillStyle = item.complete ? '#80BFFE' : '#D8F3EF'
  ...
  ctx.fillText(item.mast_name, 10, 45)
